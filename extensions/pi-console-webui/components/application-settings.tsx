...
export function ApplicationSettings({ slug }: { slug: string }) {
  ...
  const copy = async () => { try { await navigator.clipboard.writeText(`curl --request POST '${window.location.origin}/api/message/app/${app.slug}' \\\n  --header 'Content-Type: application/json' \\\n  --data '{"message":"Hello"}'`); toast("Public endpoint cURL copied."); } catch { toast("Unable to copy the public endpoint cURL.", "error"); } };
  ...
  return <div className="space-y-5">
    ...
    <Card className="grid gap-4 md:grid-cols-2">
      <div className="flex items-center justify-between gap-3 md:col-span-2"><CardTitle>{isNew ? "Create Application" : "General"}</CardTitle><div className="flex gap-2"><Button className="bg-zinc-100 text-zinc-700 hover:bg-zinc-200" title="Copy public cURL" onClick={copy}><Copy size={17}/></Button><Button className="!bg-[#2bbb77] hover:!bg-[#249b63]" title="Save Application" disabled={saving} onClick={save}>{saving ? <Loader2 className="animate-spin" size={17}/> : <Check size={17}/>}</Button></div></div>
