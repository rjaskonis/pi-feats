...
async function start() { const cfg = await config(), current = await state(); if (current && alive(current.pid)) return console.log(`Pi Console is already running at http://${current.host}:${current.port}`); if (!existsSync(join(root, "node_modules", "next"))) throw new Error(`Dependencies are missing. Run: cd ${root} && npm install`); const log = openSync(logPath(), "a"); const build = spawn(process.execPath, [join(root, "node_modules", "next", "dist", "bin", "next"), "build"], { cwd: root, stdio: ["ignore", log, log] }); const code = await new Promise<number>((resolveBuild) => build.once("exit", (value) => resolveBuild(value ?? 1))); if (code !== 0) throw new Error(`Next build failed; see ${logPath()}`); const child = spawn(process.execPath, [join(root, "node_modules", "next", "dist", "bin", "next"), "start", "--hostname", cfg.host, "--port", String(cfg.port)], { cwd: root, detached: true, stdio: ["ignore", log, log], env: process.env }); child.unref(); await writeJson(statePath(), { pid: child.pid!, host: cfg.host, port: cfg.port, startedAt: new Date().toISOString() }); console.log(`Pi Console started at http://${cfg.host}:${cfg.port}`); console.log(`Console credentials are configured in ${configPath()}.`); }
