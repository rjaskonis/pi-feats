...
export async function startApiServer(options: ServerOptions): Promise<FastifyInstance> {
  ...
  serverPromise = (async () => {
    ...
    server.get<{ Params: { slug: string }; Querystring: { path?: string } }>("/api/applications/:slug/handler", async (request, reply) => { if (!guard(request, reply)) return; registeredApplication(request.params.slug); const path = applicationHandlerPath(request.params.slug, request.query.path); return { path: request.query.path, content: await readFile(path, "utf8") }; });
