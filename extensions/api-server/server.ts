...
export async function startApiServer(options: ServerOptions): Promise<FastifyInstance> {
  ...
  serverPromise = (async () => {
    ...
    server.get<{ Params: { slug: string }; Querystring: { summary?: string; limit?: string } }>("/api/applications/:slug/logs", async (request, reply) => {
      if (!guard(request, reply)) return;
      ...
      return { calls: logs.listSummaries(limit) };
    });
