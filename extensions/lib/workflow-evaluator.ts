...
export async function evaluateWorkflowTask(ctx: ExtensionContext, input: TaskInput): Promise<TaskDecision | undefined> {
    ...
    const instruction = input.task.type === "collect"
        ? "Evaluate the recorded result against the complete collection contract: workflow source, task instruction, and acceptance criterion. The criterion is a minimum condition, never permission to weaken a more specific requirement in the instruction or workflow source. Accept only when every material requirement is demonstrably satisfied. For a request for a full name, require at least a given name and surname; a single token is incomplete. Select retry for an incomplete or incorrect response whenever another user response may satisfy the contract."
        : "Evaluate whether the recorded result demonstrates completion of this action's stated acceptance criterion. Action criteria intentionally validate the completed outcome and recorded evidence, not an independent reconstruction of every underlying calculation. Select retry only for a concrete discrepancy, omission, or contradiction in the result; do not select retry solely because independent verification confidence is limited.";
    const response = await configuredSystemOneChoice(ctx, config, input, `${instruction} Select fail only when the task cannot safely progress. Select uncertain only when the evidence cannot establish whether retry could resolve the task.`, { accept: "The result meets the task contract.", retry: "The result has a concrete, remediable discrepancy, omission, or contradiction.", fail: "The result cannot safely satisfy the task contract and the workflow must fail.", uncertain: "The evidence is insufficient to determine whether retry can resolve the task." });
    ...
    const choice = text(response.answer.choice), common = metadata(response.answer, "system_one", model);
    if (choice === "uncertain") return { outcome: "retry", reasoning: "TypeSafe System One could not determine a safe acceptance; retry the task with corrected or stronger evidence.", ...common };
    if (input.task.type === "collect" && (typeof common.confidence !== "number" || common.confidence < minimumAutomaticTaskConfidence)) return { outcome: "retry", reasoning: `TypeSafe System One confidence (${common.confidence ?? "missing"}) is below the automatic-acceptance threshold (${minimumAutomaticTaskConfidence}); retry the task with corrected or stronger evidence.`, ...common };
