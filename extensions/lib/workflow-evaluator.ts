...
const workflowRequirementPolicy = `Decide whether the supplied state requests an actual persisted Sequential Workflow execution. The keyword “Sequential Workflow” (including spelling variants) is only a trigger to evaluate; it is never evidence by itself that a workflow is required. Select a workflow route only when the user is asking the agent to actually use, create, run, continue, inspect, or cancel a Sequential Workflow to perform work. Select no workflow when the user is discussing, correcting, implementing, testing, configuring, documenting, planning, analysing, or reporting a problem about Sequential Workflow, its Skills, templates, Harness, or System One. Do not confuse a request to modify the feature with a request to execute the feature. For Skill origin, decide from both the active user request and the complete supplied Skill: a keyword in documentation, an example, a comment, frontmatter, a Skill name, or a template mention alone is not enough. Select a template route only when the supplied Skill explicitly requires creating the workflow from a named JSON template. Return uncertain only when the available state is genuinely ambiguous.`;
