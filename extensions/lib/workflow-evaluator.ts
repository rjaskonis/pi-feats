...
export async function evaluateWorkflowTask(ctx: ExtensionContext, input: TaskInput): Promise<TaskDecision | undefined> {
    ...
    const response = await configuredSystemOneChoice(ctx, config, input, "Evaluate the recorded result against the complete task contract: workflow source, task instruction, and acceptance criterion. The criterion is a minimum condition, never permission to weaken a more specific requirement in the instruction or workflow source. Accept only when every material requirement is demonstrably satisfied by the result. Check arithmetic and internal consistency when the result reports calculations: all itemized values must agree with stated totals. For a request for a full name, require at least a given name and surname; a single token is incomplete. Select retry for an incorrect, incomplete, internally inconsistent, or low-confidence result whenever another attempt could correct it. Select fail only when the task cannot safely progress. Select uncertain only when the evidence cannot establish whether retry could resolve the task.", { accept: "The result fully and demonstrably meets every material requirement of the task contract.", retry: "The result is incorrect, incomplete, internally inconsistent, or does not meet the task contract, and another attempt may succeed.", fail: "The result cannot safely satisfy the task contract and the workflow must fail.", uncertain: "The evidence is insufficient to determine whether retry can resolve the task." });
    ...
    const choice = text(response.answer.choice), common = metadata(response.answer, "system_one", model);
    if (choice === "uncertain" || typeof common.confidence !== "number" || common.confidence < minimumAutomaticTaskConfidence) return { outcome: "retry", reasoning: choice === "uncertain" ? "TypeSafe System One could not determine a safe acceptance; retry the task with corrected or stronger evidence." : `TypeSafe System One confidence (${common.confidence ?? "missing"}) is below the automatic-acceptance threshold (${minimumAutomaticTaskConfidence}); retry the task with corrected or stronger evidence.`, ...common };
