...
const systemOneRequirement = async (ctx: ExtensionContext, config: WorkflowEvaluationConfig, input: RequirementInput): Promise<RequirementDecision> => {
    ...
    const response = await typesafeChoice(ctx, model, input, "Which workflow route is appropriate? Do not treat wording, names, frontmatter, or a template mention as conclusive by themselves.", options);
...
export async function evaluateWorkflowTask(ctx: ExtensionContext, input: TaskInput): Promise<TaskDecision | undefined> {
    ...
    const response = await typesafeChoice(ctx, model, input, "Does the recorded result meet the task criterion? Select fail only when the task cannot safely progress; select retry when another attempt may meet the criterion.", { accept: "The result fully meets the criterion.", retry: "The result does not meet the criterion but another attempt may succeed.", fail: "The result cannot safely satisfy the criterion and the workflow must fail.", uncertain: "The evidence is insufficient for a safe transition." });
