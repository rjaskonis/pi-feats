...
export function workflowHarness(host: ExtensionAPI, db: DatabaseSync) {
    ...
    const workflowOperation = /\b(?:use|create|run|start|continue|inspect|cancel|execute|inicie|crie|rode|continue|inspecione|cancele|execute|usar|criar|rodar|iniciar|executar)\b/i;
    const explicitWorkflowRequest = (text: string) => workflowKeyword.test(text) && workflowOperation.test(text);
    ...
    let definitionCutoverReadPending = false;
    ...
    host.on("turn_start", () => { dispatched = false; definitionCutover = false; definitionCutoverOrigin = undefined; definitionCutoverReadPending = false; });
            ...
            try {
                ...
                const content = await readFile(path, "utf8");
                {
                    ...
                    if (decision.status === "required") {
                        ...
                        definitionCutoverReadPending = true;
                        return;
                    }
    ...
    host.on("tool_result", (event) => {
        if (definitionCutoverReadPending && (event as any).toolName === "read" && !(event as any).isError) {
            definitionCutoverReadPending = false;
