...
export default function registerConversationSearch(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    description: "Search previous conversations in the active profile by topic or wording. Use this when the user asks what was discussed in another conversation or a previous session; the user does not need to know a session ID. Searches only this profile and never other profiles.",
    promptSnippet: "Search previous conversations in the active profile by topic; no session ID is required.",
    promptGuidelines: ["Use conversation_search when the user asks about information discussed in another conversation, a prior session, or asks you to search previous chats. Do not ask for a session ID before searching by topic."],
    parameters: Type.Object({ query: Type.String({ minLength: 2, description: "Topic, phrase, or information to search for in prior conversations." }), limit: Type.Optional(Type.Integer({ minimum: 1, maximum: 10, description: "Maximum number of matching conversations to return." })) }),
    async execute(_id, params) {
      ...
      const results = searchConversationEntries(params.query, candidates, params.limit ?? 5);
      ...
      return { content: [{ type: "text" as const, text }], details: { query: params.query, searchedSessions: sessions.length, matches: results.map((result) => ({ id: result.id, name: result.name ?? null, updatedAt: result.updatedAt.toISOString(), score: result.score })) } };
    },
  });
}
