...
test("detected Sequential Workflow requirement creates a pending workflow before requested work", async () => {
    const h = await setup();
    try {
...
test("a Skill requirement is decided by the configured evaluator", async () => {
    ...
    try {
        ...
        assert.deepEqual(h.activeTools(), ["read", "sequential_workflow_create_from_template"]);
        const skillResult = { role: "tool", content: "Create the Sequential Workflow from contract.workflow.json." };
        const context = await h.hook("context_with_system", { messages: [skillResult] });
        assert.match(context.messages[0].content, /The Skill just read specifies the template/);
        assert.doesNotMatch(context.messages[0].content, /contract\.workflow\.json/);
        assert.deepEqual(context.messages[1], skillResult);
        const readResult = await h.hook("tool_result", { toolName: "read", toolCallId, content: [{ type: "text", text: "Skill body" }], isError: false });
        assert.equal(readResult, undefined);
        assert.equal((await h.hook("tool_call", { toolName: "bash", toolCallId: "other-call", input: {} })).block, true);
        assert.equal(await h.hook("tool_call", { toolName: "sequential_workflow_create_from_template", toolCallId: "create-call", input: { path: templatePath } }), undefined);
