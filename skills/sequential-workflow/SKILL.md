...
`sequential_workflow_create` creates only root workflows and hydrates a focused `pending_definition`; it has no parent parameters and never creates a child. `sequential_workflow_create_subworkflow` creates the hierarchical relationship and operational dependency for a current `workflow` task. Use `sequential_workflow_status` without an ID to list this session's active workflows or with `workflowId` to inspect an owned execution. Status never changes focus. When the user requests cancellation, call `sequential_workflow_cancel` with the workflow ID. Cancellation is available across sessions in the same profile and does not require adoption. It cancels only that workflow; a cancelled child is surfaced to its parent for evaluation and descendants are not cancelled automatically. `/workflow-cancel <workflowId>` remains available as an explicit user control.
